...
from resnet.resnet_file import ResNet18_Weights
from resnet.resnet_file import resnet18_custom
from resnet.utils_resnet import ImageNetProcessor
...
def load_model():
    ...
    model = resnet18_custom(weights=ResNet18_Weights.IMAGENET1K_V1)
...
def evaluate_model(model, processor):
    ...
    topk_accuracy = processor.accuracy_top2(outputs, all_labels)
    print(f"Accuracy of the ResNet18 model on the images: {accuracy:.2f}%")
    print(f"Top-1 Accuracy of the ResNet18 model on the images: {topk_accuracy:.2f}%")
...
def compile_model(
    ...
    print(f"Input shape: {images.shape}")  # Assicurati che sia [N, C, H, W] con N > 0

    if compile_func is compile_torch_model:
...
def run_fhe_execution(q_module, images, num_images=1):
    ...
    def get_top5_labels(output):
        return np.argsort(output.flatten())[-2:][::-1]
...
def run_resnet18_fhe(
    ...
    # importa qui anche le tue funzioni load_model, ImageNetProcessor, evaluate_model, run_experiment, compile_model,
    # export_statistics, evaluate_model_cml, run_fhe_execution oppure falli importare nel file che richiama questa funzione

    if use_gpu and not concrete.compiler.check_gpu_available():
        use_gpu = False
    ...
    resnet18 = load_model().to(device)
    for i, param in enumerate(resnet18.parameters()):
        param_ = torch.from_numpy(weights[i]).to(device)
        param.data.copy_(param_)
    calib_images = processor.get_calibration_tensor().to(device)
    ...
    if run_exp:
        ...
        q_module = compile_model(
            ...
            fhe_mode="simulate",
            compilation_device=(
                "cuda" if use_gpu and concrete.compiler.check_gpu_available() else "cpu"
            ),
        )
    ...
    if run_fhe:
        ...
        print("FHE execution was not run. Use run_fhe=True to enable it.")
